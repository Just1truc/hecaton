...
def provider_call(provider, method: str, args: Tuple):
    try:
...
async def startup_check():
    ...
    if not admin_user:
        ...
        if admin_pass:
            ...
            print("Created admin user from environment variable.")
        else:
            print(
                "WARNING: No admin user found. You should create one or set HECATON_ADMIN_PASS env var."
            )
...
def create_user(user_dto: NewUserDTO):
    uid = q.create_user(
        user_dto.username, get_password_hash(user_dto.password), user_dto.role
    )
    if not uid:
...
def update_job(update_dto: JobUpdateDTO):
    ...
    return provider_call(
        q,
        "update_job",
        (update_dto.job_id, update_dto.new_status, update_dto.new_payload),
    )
...
def connect_worker(worker_dto: WorkerConnectionDTO):
    return {
        "worker_id": provider_call(
            q, "connect_worker", (worker_dto.worker_id, worker_dto.gpu_name)
        )
    }
...
def update_worker(worker_update_dto: WorkerStatusUpdateDTO):
    provider_call(
        ...
        (worker_update_dto.worker_id, worker_update_dto.status),
    )

...
from hecaton.client.managers.server import ServerInfo, ServerManager

image_app = typer.Typer()


class ImageManager:
    def list_images(self, ip, secret):
        return HecatonServer.list_images(ip, secret)

    def new_image(self, ip, secret, image):
...
def list_image(ctx: typer.Context):
...
def new_image(ctx: typer.Context, image: str):
    mgr: ImageManager = ctx.obj["image_mgr"]
    server_mgr: ServerManager = ctx.obj["server_mgr"]
    server_info: ServerInfo = server_mgr.connected_server()
...
def update_image(
    ...
    env_file_path: Optional[str] = typer.Option(
        None,
        "--fp",
        callback=prompt_optional(
            "    New_env_path (Press Enter to keep current variables)"
        ),
        help="Env Path",
        show_default=False,
    ),
    ...
    if env_file_path and not os.path.isfile(env_file_path):
        ...
        env = [
            line.split("=")
            for line in open(env_file_path, "r").read().split()
            if "=" in line
        ]

    typer.echo(
        mgr.update_image(server_info.ip, server_info.secret, image, env, description)
    )

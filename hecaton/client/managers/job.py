...
class JobManager:
    def list_jobs(self, ip, secret):
        return HecatonServer.list_jobs(ip, secret)

    def new_job(self, ip: str, secret: str, file: str, image: str):
        return HecatonServer.new_job(ip, secret, file, image)

    def show_job(self, ip: str, secret: str, jid: str):
...
def show_job(
    ...
    id, status, payload, index, _ = mgr.show_job(
        server_info.ip, server_info.secret, job_id
    )

...
from hecaton.client.managers.db import ServerInfo, with_locked_db
...
class ServerManager:
    ...
    def register_server(self, ip: str, name: str, secret: str):
        with with_locked_db(mutate=True) as db:
            found = False
    ...
    def server_name_exists(self, server_name: str):
...
def login(
    ...
    password: str = typer.Option(
        ..., prompt="    Password", hide_input=True, help="Password"
    ),
):
    mgr: ServerManager = ctx.obj["server_mgr"]
...
def server_connect(
    ...
    if not mgr.server_name_exists(server_name):
        typer.echo(
            "error: Unknown server. Do 'server list' to show registered servers."
        )
    else:
        mgr.selected_server = server_name
...
def server_info(ctx: typer.Context, server_name: str):
    mgr: ServerManager = ctx.obj["server_mgr"]
    ...
    typer.echo(
        f"Auth: \t{'JWT' if server.token else ('Secret' if server.secret else 'None')}"
    )


@server_app.command("disconnect")
def server_disconnect(ctx: typer.Context):
    mgr: ServerManager = ctx.obj["server_mgr"]

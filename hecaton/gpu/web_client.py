...
from hecaton.gpu.utils import WorkerConfig, load_worker_config, save_worker_config
...
class GPUWebClient:
    ...
    def __connect_server(self, gpu_name: str | None = None):
        ...
        result = requests.post(
            f"{ip}/workers/connect", json=payload, headers=self.headers
        )
        if not (result.ok):
            raise RuntimeError(
                f"Failed to connect to server {self.ip}. Cause: {result.json()['detail']}"
            )
    ...
    def update_status(self, status: str):
        ...
        if not (result.ok):
            raise RuntimeError(
                f"Failed to update worker status {result.json()['detail']}"
            )

    def update_job_status(self, jid: str, status: str):
        ip = self.ip if self.ip.startswith("http") else f"https://{self.ip}"
        result = requests.post(
            f"{ip}/jobs/update",
            headers=self.headers,
            json={"job_id": jid, "new_status": status},
        )
        if not (result.ok):
            raise RuntimeError(
                f"Failed to update worker status {result.json()['detail']}"
    ...
    def update_job(self, jid: str, status: str, payload: dict):
        ip = self.ip if self.ip.startswith("http") else f"https://{self.ip}"
        result = requests.post(
            f"{ip}/jobs/update",
            headers=self.headers,
    ...
    def job_assigned(self):
        ...
        try:
            ...
            if not (result.ok):
                raise RuntimeError(
                    f"Failed to fetch worker job {result.json()['detail']}"
                )
